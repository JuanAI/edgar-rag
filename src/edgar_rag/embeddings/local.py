...
who checks out the repo. The cost is a one-time model download (~90 MB) and real
compute per batch, in exchange for embeddings that actually capture meaning
("revenue" lands near "sales").
...
class LocalEmbeddingProvider:
    ...
    def dim(self) -> int:
        ...
        # `get_embedding_dimension`; prefer the new name, fall back to the old.
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        # normalize_embeddings=True returns unit vectors so the search index can
