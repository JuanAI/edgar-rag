...
def get_embedding_provider(settings: Settings) -> EmbeddingProvider:
    ...
    if provider == "local":
        # Imported lazily: sentence-transformers is an optional dependency, and
        # the model itself is only loaded on the first embed call.
        from .local import LocalEmbeddingProvider

        return LocalEmbeddingProvider(settings.embedding_model)
    raise ValueError(f"unknown embedding_provider {provider!r}; supported providers: 'local'")
